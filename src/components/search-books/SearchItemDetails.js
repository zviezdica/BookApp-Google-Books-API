...
const SearchItemDetails = ({data, closeDetails}) =>{
    console.log(data)
    ...
    return(
        <div className="details-card shadow-card flex flex-col container max-h-70vh bg-white fixed overflow-y-auto top-100 xs:top-130 lg:top-180 left-1/2 -translate-x-1/2 p-10">
            <div>
                <div style={{backgroundImage:`url(${close})`}} className='h-40 w-40 bg-center bg-cover bg-no-repeat opacity-50 hover:opacity-100 transition-opacity mr-0 ml-auto cursor-pointer' onClick={closeDetails}></div>
