...
const ResultItem = ({item, handleDetails}) =>{
    ...
    return(
            ...
            <div className="book-card" onClick={()=>handleDetails(id)}>
                ...
                {authors && <h4 className="text-14 text-center pb-5">{authors.toString().replace(',', ', ')}, {publishedDate && publishedDate.slice(0,4)}</h4>}
                {imageLinks && <img className="rounded-sm w-100 my-20" src={imageLinks.smallThumbnail}></img>}
                <div className="see-more mb-10 capitalize py-2 px-15 md:px-15 text-peacock-blue text-17 transition-all duration-200 opacity-0">see more</div>
            </div>
